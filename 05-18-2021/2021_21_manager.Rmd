...
survey_fm %>%
  group_by(overall_years_of_professional_experience,industry) %>%
  summarise(total_comp_avg = mean(total_comp)) %>%
  ungroup() %>%
  arrange(total_comp_avg) %>%
  # mutate(overall_years_of_professional_experience = fct_reorder(overall_years_of_professional_experience, desc(annual_salary))) %>%
  ...
  ggplot(aes(total_comp_avg, y=factor(overall_years_of_professional_experience), fill=industry)) +
  geom_col() +
  theme_grey() +
